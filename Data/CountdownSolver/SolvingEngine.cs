...
internal sealed class SolvingEngine
{
    ...
    /// a list of results for all solutions
    public List<(int calcResult, string calcSummary)> AllSolutions { get; } = new List<(int, string)>();


    /// a list of results equaling the target
    public List<string> TargetSolutions { get; } = new List<string>();
    ...
    public SolvingEngine(int? target)
    {
        ...
        // ensure capacity
        AllSolutions = new List<(int, string)>(250);
        this.target = target;
    }
    ...
    /// </summary>
    public void SolveAll(ReadOnlySpan<int> permutation)
    {
    ...
    public void SolveTarget(ReadOnlySpan<int> permutation)
    {
        foreach (int[] mapEntry in PostfixMap.Instance[permutation.Length])
            SolveTargetRecursive(-1, mapEntry, 0, permutation, 0, 0);
    }
    ...
    private void SolveRecursive(int stackHead, ReadOnlySpan<int> mapEntry, int mapIndex, ReadOnlySpan<int> permutation, int permutationIndex, int depth)
    {
        ...
        for (int op = cMultiply; op <= cDivide; op++)
        {
            ...
            if (result > cInvalidResult)  // valid result
            {
                ...
                {
                    if (result > 99 && result < 1000) //any integer between 100 and 999 inclusive is valid
                    {
                        AllSolutions.Add((result, ConvertToString(mapEntry, permutation)));
                        //break; //we don't break any longer as we may find more answers recursively
                    }
                }
            }
        }
    ...
    /// <param name="permutationIndex">position in the permutation</param>
    /// <param name="depth">recursion depth</param>
    private void SolveTargetRecursive(int stackHead, ReadOnlySpan<int> mapEntry, int mapIndex, ReadOnlySpan<int> permutation, int permutationIndex, int depth)
    {
        const int cInvalidResult = 0;
        ...
        for (int op = cMultiply; op <= cDivide; op++)
        {
            ...
            if (result > cInvalidResult)  // valid result
            {
                ...
                if (mapIndex < mapEntry.Length)   // some left
                {
                    ...
                    // evaluate the next sequence
                    SolveTargetRecursive(stackHead, mapEntry, mapIndex, permutation, permutationIndex, depth + 1);
                }
                else
                {
                    if (result == target)   // got one...
                    {
                        TargetSolutions.Add(ConvertToString(mapEntry, permutation));
                        break;
                    }
